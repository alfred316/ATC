...
//__global__ void CheckCollisionPath(float *x, float *y, float *dx, float *dy, int *col, float *timeTill, int * colWith)
//{
//	int i = threadIdx.x;
...
//			{
//
//				angle[i] = angleBase + (angleBase * chk);
...
//				//upper bound
//				upx[i] = batx[i] + bound + (t * 120 * sdx[i]);
//				upy[i] = baty[i] + bound + (t * 120 * sdy[i]);
//				__syncthreads();
//				//lower bound
//				lowx[i] = batx[i] - bound + (t * 120 * sdx[i]);
//				lowy[i] = baty[i] - bound + (t * 120 * sdy[i]);
//				__syncthreads();
//
...
//						tmpX = lowx[p] - upx[i];
//						minX = fabsf(tmpX);
//						minX = minX / fabsf(sdx[p] - sdx[i]);
//						//get max_x
//						tmpX = upx[p] - lowx[i];
//						maxX = fabsf(tmpX);
//						maxX = maxX / fabsf(sdx[p] - sdx[i]);
//
//						//get min_y
//						tmpY = lowy[p] - upy[i];
//						minY = fabsf(tmpY);
//						minY = minY / fabsf(sdy[p] - sdy[i]);
//						//get max_y
//						tmpY = upy[p] - lowy[i];
//						maxY = fabsf(tmpY);
//						maxY = maxY / fabsf(sdy[p] - sdy[i]);
//
...
int main(void)
{
	...
	for (;;)
	{
		...
		if (count == 0 || count == half_dur || count == almostDone)
		{
			...
			//CheckCollisionPath << <blocks, threads >> >(drones->x, drones->y, drones->dx, drones->dy, drones->col, drones->timeTill, drones->colWith);
			...
			cudaMemcpy(h_timeTill, d_timeTill, size, cudaMemcpyDeviceToHost);
			cudaMemcpy(h_colWith, d_colWith, size, cudaMemcpyDeviceToHost);
